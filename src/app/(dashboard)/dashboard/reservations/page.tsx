        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Manage hotel reservations and bookings</p>
                </div>
                <Link href="/dashboard/reservations/new">
                    <Button className="bg-indigo-600 hover:bg-indigo-700"><Plus className="w-4 h-4 mr-2" />New Booking</Button>
                </Link>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
                {STATUS_TABS.map(tab => (
                    <button key={tab.value} onClick={() => setStatusFilter(tab.value)}
                        className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${statusFilter === tab.value ? tab.color + ' ring-2 ring-offset-1 ring-indigo-300' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'
                            }`}>
                        {tab.label}
                    </button>
                ))}
            ...
            <div className="relative max-w-sm">
                ...
                <Input placeholder="Search reservation number..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
            </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b bg-slate-50/80">
                                    <th className="text-left p-4 font-semibold text-slate-600">Reservation</th>
                                    <th className="text-left p-4 font-semibold text-slate-600">Guest</th>
                                    <th className="text-left p-4 font-semibold text-slate-600">Room</th>
                                    <th className="text-left p-4 font-semibold text-slate-600">Dates</th>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr><td colSpan={7} className="p-12 text-center text-slate-400">No reservations found</td></tr>
                                ) : (
                                    data.map(r => {
                                        ...
                                        return (
                                            <tr key={r.id} className="border-b hover:bg-slate-50/50 transition-colors">
                                                <td className="p-4">
                                                    <Link href={`/dashboard/reservations/${r.id}`} className="font-mono font-semibold text-indigo-600 hover:text-indigo-800 text-xs">
                                                ...
                                                <td className="p-4">
                                                    <div className="text-slate-700">{room?.room_number || <span className="text-slate-400 italic">Unassigned</span>}</div>
                                                    <div className="text-xs text-slate-400">{roomType?.name}</div>
                                                </td>
                                                <td className="p-4">
                                                    <div className="flex items-center gap-1.5 text-slate-600">
                                                        <CalendarDays className="w-3.5 h-3.5" />
                                                        <span className="text-xs">{new Date(r.check_in_date).toLocaleDateString('en-GB')} — {new Date(r.check_out_date).toLocaleDateString('en-GB')}</span>
                                                    </div>
                                                    <div className="text-xs text-slate-400 mt-0.5">{nights} night{nights > 1 ? 's' : ''}</div>
                                                </td>
                                                <td className="p-4">
                                                    <div className="font-medium text-slate-800">฿{Number(r.rate).toLocaleString()}</div>
                                                    <div className="text-xs text-slate-400 flex items-center gap-1"><Users className="w-3 h-3" />{r.adults}A{r.children > 0 ? ` ${r.children}C` : ''}</div>
                                                </td>
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="flex justify-end gap-2 mt-4">
                        ...
                        <Button onClick={handleAction} disabled={actionLoading}
                            className={actionDialog?.type === 'cancel' ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}>

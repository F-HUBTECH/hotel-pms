 ...
 *
 * Note: Service charge is calculated from pre-VAT amount
 ...
 */
function calculateVATTypeA(
  gross: number,
  vatRate: number = 7.00,
  servRate: number = 10.00
) {
  // VAT Amount
 ...
 * @param servRate - Service charge percentage
 * @returns Calculated breakdown
 */
function calculateVATTypeB(
  net: number,
  vatRate: number = 7.00,
  servRate: number = 10.00
) {
 ...
 * @returns Breakdown with zero VAT/SC
 */
function calculateNonVAT(amount: number) {
  return {
    gross: amount,
    netAmount: amount,
    vatAmount: 0,
